...
### 2. Observer Pattern / Event-Driven Scaffolding
...
**Implementation**: The `ParkingLot` class was upgraded to a generic Event Publisher. It exposes `add_event_observer()` and `remove_event_observer()` methods. Instead of passing brittle strings, the `ParkingLot` publishes strongly-typed `DomainEvent` objects (like `VehicleParkedEvent` and `VehicleDepartedEvent`). The GUI subscribes to these events and displays them, while `ParkingLot` remains entirely decoupled from presentation concerns. This provides the scaffolding necessary for a true Event-Driven Architecture (EDA) where future microservices (like Billing or Analytics) can listen directly to these domain events.

### 3. Strategy Pattern
In the original application, parking fees were not explicitly modeled or calculated when vehicles left the lot. As the system expanded to support dynamic pricing models based on vehicle types and EV surcharges, hardcoding this logic directly into the `ParkingLot` would have caused a violation of the Single Responsibility Principle and Open/Closed Principle.

**Implementation**: We introduced the `PricingStrategy` interface (abstract base class) and several concrete implementations (`FlatRateStrategy`, `EVPremiumStrategy`, and `VehicleTypeStrategy`). The `ParkingLot` now accepts a pricing strategy upon initialization (or can have it hot-swapped dynamically via `set_pricing_strategy()`), and delegates the fee calculation to it during the `leave()` use case. This allows the facility management to seamlessly switch from flat-rate billing to dynamic EV-surcharge billing without changing the core parking logic.
